...
fn main() {
    ...
    let cloneOne = xs[0].clone();
    let cloneTwo = xs[1].clone();

    let miniSumOne = thread::spawn(move || map_data(&cloneOne));
    let miniSumTwo = thread::spawn(move || map_data(&cloneTwo));

    intermediate_sums.push(miniSumOne.join().unwrap());
    intermediate_sums.push(miniSumTwo.join().unwrap());
    ...
    let finalSum = reduce_data(&intermediate_sums);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut xxs: Vec<Vec<usize>> = Vec::new();
    let mut index = 0;

    for _ in 0..num_partitions{
        let mut vect: Vec<usize> = Vec::new();
        let mut partSize = partition_size;
        if remainder > 0{
            partSize += 1;
            remainder -= 1;
        }
