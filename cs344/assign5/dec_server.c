...
//Function that takes the ciphertext and keytext, and decrypts it into the plaintext
int decrypt(char* plainText, char* cipherText, char* keyText){
    int num, i = 0;
    while(cipherText[i] != '\0'){
        int cipher = ctoi(cipherText[i]);
...
//Main function that runs the decryption server
    ...
    while(1){
        ...
        switch(pid){
            ...
            case 0:
                ...
                //Recieve message from client with length
                ...
                //Split the different strings that came form that client
                token = strtok_r(buffer, "\n", &contextptr);
                strcpy(cipherText, token);
                token = strtok_r(NULL, "\n", &contextptr);

                //Decrypt the ciphertext into the plaintext
                sendSize = decrypt(plainText, cipherText, token);
                ...
                j = 0;
                while(sent < sendSize){
                    j = send(connectSock, plainText, sendSize, 0);
                    if(j == -1){
                        perror("ERROR on data reception");
                        exit(EXIT_FAILURE);
                    }
