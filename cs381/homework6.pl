% Group members:
%  * Nat Bourassa 933683505
%  * Jackson E. Rollins 932932878
%  * Joseph Goodwin 932544756
%
...
related(X, Y) :- class(X, C), class(Y, C), X \= Y.
...
competitor(X, Y, F) :- diet(X, F), diet(Y, F), neighbor(X, Y).
...
would_eat(X, Y) :- diet(X, D), class(Y, D).
...
cmd(H, L, [H|L]) :- string(H).
cmd(H, L, [H|L]) :- number(H).
cmd(tru, L, [tru|L]).
cmd(fls, L, [fls|L]).
cmd(dup, [H|L], [H,H|L]).
cmd(swap, [X,Y|L], [Y,X|L]).
cmd(add, [X,Y|L], [Z|L]) :- Z is X + Y.
cmd(mul, [X,Y|L], [Z|L]) :- Z is X * Y.
cmd(lte, [X,Y|L], [tru|L]) :- X >= Y.
cmd(lte, [X,Y|L], [fls|L]) :- X < Y.
cmd(if(X, _), [tru|L], S) :- prog(X, L, S).
cmd(if(_, Y), [fls|L], S) :- prog(Y, L, S).


% 2. Define the predicate `prog/3`.
prog([], L, L).
prog([H|T], L, S) :- cmd(H, L, Z), prog(T, Z, S).
