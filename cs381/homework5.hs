-- Group members:
--  * Name Nat Bourassa, ID 933683505
--  * Name Jackson E. Rollins, ID 932932878
--  * Name Joseph Goodwin, ID 932544756
--
...
checkCmd map v (Pen mode)         = True
checkCmd map v (Move x y)         = checkExpr v x && checkExpr v y
checkCmd map v (Call macro args) =  case lookup macro map of
                                   Just n -> length args == n && all (checkExpr v) args
                                   Nothing -> False     
checkCmd map v (For var l r block) = checkExpr v l && checkExpr v r && checkBlock map (var:v) block



checkExprList :: [Var] -> [Expr] -> Bool
checkExprList v []     = True
checkExprList [] e     = True
checkExprList v (x:xs) = checkExpr v x && checkExprList v xs
...
checkBlock [] [] [] = True
--checkBlock map v (x:xs) = case checkCmd map v x of
--                         True -> checkBlock map v xs
--                         False -> False
checkBlock i v block = all (checkCmd i v) block
...
checkDef map (Define macro pars block) = checkBlock map pars block 
...
expr :: Env -> Expr -> Int
expr e (Lit n) = n
expr e (Ref n) = case lookup n e of
                                 Nothing -> 0
                                 Just x -> x
...
cmd defs env state@(pen,pos) c = case c of
    ...
    Call macro args -> case lookup macro defs of
                    Just (pars,aBlock) -> block defs (setAll pars (map (expr env) args) []) state aBlock
