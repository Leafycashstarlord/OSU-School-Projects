...
prettyExpr (Mul l r) =  (helper l) ++ " * "  ++ (helper r) 
prettyExpr (Ref x) = x
prettyExpr (Lit i) = show i

helper :: Expr -> String
helper (Add l r) = "(" ++ (prettyExpr (Add l r)) ++ ")"
helper x = prettyExpr (x)
...
prettyCmd (Move l r) = "move(" ++ (prettyExpr l)++ ", " ++ (prettyExpr r) ++ ")"
prettyCmd (Call l r) = l ++ "(" ++ intercalate ", " (map prettyExpr r) ++ ")"
prettyCmd (For i l r b) = "for " ++ i ++ " = "++ (prettyExpr l) ++ " to " ++ (prettyExpr r) ++ " " ++ prettyBlock b
