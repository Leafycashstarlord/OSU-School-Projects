...
GLSLProgram* Pattern;
bool	AnimateVshader = true;
bool	AnimateFshader = true;
float	Timevert = -1;
float	Timefrag = -1;
...
Display( )
{
	...
	Pattern->Use();
	if (AnimateVshader == true) {
		Timevert = 0.5 + 0.5 * sin(2. * M_PI * Time);
	}
	Pattern->SetUniformVariable("vTime", Timevert);

	if (AnimateFshader == true) {
		Timefrag = 0.5 + 0.5 * sin(2. * M_PI * Time);
	}
	Pattern->SetUniformVariable("fTime", Timefrag);
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'v': case 'V':
			// Vertex shader only
			AnimateVshader = true;
			AnimateFshader = false;
			break;

		case 'F':
			// Fragment shader only
			AnimateVshader = false;
			AnimateFshader = true;
			break;
