...
Animate( )
{
	...
	int ms = glutGet(GLUT_ELAPSED_TIME);			// milliseconds since the program started				// milliseconds in the range 0 to MS_IN_THE_ANIMATION_CYCLE-1
...
Display( )
{
	...
	*/
	...
	xyz lightZeroPos = {1, -1, 1};
	glPushMatrix();
	glTranslatef(-2 * sin(Time), -1, 2 * cos(Time));
	glColor3f(1, 1, 1);
	glutSolidSphere(0.05, 50, 50);
	SetPointLight(GL_LIGHT0, lightZeroPos.x, lightZeroPos.y, lightZeroPos.z, 1, 1, 1);
	glPopMatrix();

	xyz lightOnePos = { 3, 2, 0};
	glPushMatrix();
	glTranslatef(lightOnePos.x, lightOnePos.y, lightOnePos.z);
	glColor3f(0, 0, 1);
	glutSolidSphere(0.05, 50, 50);
	SetSpotLight(GL_LIGHT1, lightOnePos.x, lightOnePos.y, lightOnePos.z, -lightOnePos.x, -lightOnePos.y, -lightOnePos.z, 0, 0, 1);
	glPopMatrix();

	xyz lightTwoPos = { -1, 3, 0 };
	glPushMatrix();
	glTranslatef(lightTwoPos.x, lightTwoPos.y, lightTwoPos.z);
	glColor3f(0, 0, 0);
	glutSolidSphere(0.05, 50, 50);
	SetPointLight(GL_LIGHT2, lightTwoPos.x, lightTwoPos.y, lightTwoPos.z, 0, 1, 0);
	glPopMatrix();
	...
	SetMaterial(1, 0, 0, 1);
	glTranslatef(2, 2, 0);
	OsuSphere(radius/1.99, slices / 2, stacks / 2);
	glPopMatrix();
	...
	glTranslatef(2*sin(Time), 0, 2*cos(Time));
	OsuSphere(radius/1.99, slices, stacks);
