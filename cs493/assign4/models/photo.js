...
exports.savePhotoFile = function (photo) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        fs.createReadStream(photo.path).pipe(uploadStream).on('error', function (err) {
            reject(err)
        })
...
exports.removeUploadedPhoto = function(photo) {
    return new Promise( function (resolve, reject) {
        fs.unlink(photo.path).on('error', function (err) {
            reject(err)
        }).on('finish', function (result){
            resolve()
        })
...
async function getPhotosByBusinessId(businessId) {
    ...
    if(!ObjectId.isValid(businessId)) {
        ...
        const results = await bucket.find({ 'metadata.businessId': new ObjectId(businessId) }).toArray()
        return results
    }
}
