...
int get_queue_data(struct dynarray* da){
  for(int i = 0; i < da->size; i++){
    if(da->data[i] != NULL){
      return 0;
    }
...
int get_dynarray_back(struct dynarray* da){
  return da->back;
}

int get_dynarray_front(struct dynarray* da){
  return da->front;
}

void remove_dynarray_front(struct dynarray* da){
