...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  struct link* currentLink = list->head;
  struct link* prevLink = list->head;

  while(currentLink != NULL){
    if(cmp(val, currentLink->val) == 0){
      if(currentLink == list->head){
        list->head = list->head->next;
      }
